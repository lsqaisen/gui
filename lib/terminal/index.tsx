...
export default class extends React.PureComponent<TerminalProps, any> {
  ...
  componentDidMount() {
    const { resize, bidirectional, splitWrite, successMessages, buffered, beforeSendData } = this.props;
    this.socket!.onopen = () => {
      this.term!.write(`                              \r`);
      this.term!.write(successMessages || "\x1b[32mConnected successfully!\n\r\x1b[0m");
  ...
  render() {
    ...
    return (
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, padding: 16, paddingRight: 0, backgroundColor: '#000' }}>
        {isWindows ? <div
          style={{
            position: 'absolute',
            padding: 8,
