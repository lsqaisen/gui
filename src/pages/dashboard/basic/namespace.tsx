  ...
  return (
    <Chart
      ...
      onDraw={(chart: G2.Chart) => {
        ...
        chart.interval().position('namespace*total').color('type')
          .adjust([{
            type: 'dodge',
            marginRatio: 1 / 32
          }]);
...
const Namespaces = ({ loading, apps, services, nslist, dispatch }: NamespacesProps) => {
  const [names, setNs] = useState([]);
  const getNSs = () => dispatch({ type: 'namespace/get' }).then((data: any) => {
    names.length <= 0 && data.items.length > 0 && setNs(data.items.map((v: INs) => v.metadata.name));
  });
  const getServices = (ns: string) => dispatch({ type: 'apps/services', payload: ns });
          ...
          <Row>
            <Col span={24}>
              <Select
                ...
                onChange={setNs}
              >
                {nslist.map(v => (<Select.Option key={v.metadata.name}>{v.metadata.name}</Select.Option>))}
              </Select>
            </Col>
            <Col span={24}>
