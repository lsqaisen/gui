...
const Users = ({ loading, total = 0, audits, dispatch }: UsersProps) => {
  ...
  const getAudits = () => dispatch({ type: 'users/audits', payload: { page: 1, size: 10, query: "" } });
  useEffect(() => { getUsers(); getAudits() }, []);
  return (
    <>
      ...
      <section className="box" style={{ marginBottom: '24px' }}>
        <Skeleton loading={loading}>
          <PageHeader title="审计日志" subTitle={`最近10条(总共${audits.total}条)`} style={{ padding: 0 }} extra={<Link to="/user/audits">详情</Link>}>
            <List
              dataSource={audits.items}
              locale={{ emptyText: '暂无审计日志' }}
              renderItem={(audit) => (
                <Typography.Paragraph style={{ wordBreak: 'break-all', color: audit.status !== 200 ? '#ff5242' : '' }} ellipsis={{ rows: 2, expandable: true }}>
                  对象：<Typography.Text mark>{audit.repo_name}</Typography.Text> -> <Typography.Text mark>{audit.resource}</Typography.Text>,
                记录：{audit.operation}
