...
import Table, { VersionsProps as Props } from "@/components/apps/app/detail/versions";
...
const Versions = Wapper((namespace, name, apps) => ({
  ...
  ({ type, name, namespace, data, dispatch, ...props }) => {
    const getVersions = () => dispatch({ type: 'apps/versions', payload: { name, namespace, type } });
    useEffect(() => { getVersions() }, [type, name, namespace]);
    return (
