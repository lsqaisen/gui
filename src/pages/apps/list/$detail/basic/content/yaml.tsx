...
}))<YamlProps>(({ type, name, namespace, yaml, dispatch, ...props }: YamlProps) => {
  const getYaml = () => dispatch({ type: 'apps/export', payload: { name, namespace, type } });
  useEffect(() => { getYaml() }, [type, namespace])
  return (
    <section style={{ height: yaml === undefined ? 160 : "auto", padding: '24px 0' }}>
