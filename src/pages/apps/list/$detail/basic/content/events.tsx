...
}))<EventsProps>(({ current, type, name, namespace, data, dispatch, onError, ...props }: EventsProps) => {
  const getEvents = () => dispatch({ type: 'apps/events', payload: { name, namespace, type } });
  useEffect(() => { getEvents() }, [type, name, namespace]);
