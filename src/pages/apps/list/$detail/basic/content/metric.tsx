...
import Table, { MetricsProps as Props } from "@/components/apps/app/detail/metric";
...
}))<MetricsProps>(({ current, type, name, namespace, data, dispatch, onError, ...props }: MetricsProps) => {
  const getMetrics = () => dispatch({ type: 'apps/metrics', payload: { name, namespace, type } });
  useEffect(() => { getMetrics() }, [type, name, namespace]);
  console.log(data)
  return (
