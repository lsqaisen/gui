...
const Layout: React.FC<LayoutProps> = ({ns, dispatch, children}) => {
  ...
  const deletePod = (app: App) =>
    dispatch({type: 'apps/deletePod', payload: app});
  const modifyService = (app: ModifyServiceType) =>
    dispatch({type: 'apps/modifyService', payload: app});
  const importApp = (data: ImportAppType) =>
    dispatch({type: 'apps/import', payload: data});
  const getImages = (payload: getImagesRequest) =>
  ...
  const createSecret = (payload: SecretRequest) =>
    dispatch({type: 'secret/create', payload});
  const modifyReplicas = (payload: ModifyReplicasRequest) =>
    dispatch({type: 'apps/modifyReplicas', payload});
  return (
    <Context.Provider
      value={{
        getImages,
