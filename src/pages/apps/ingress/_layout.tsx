...
const Layout: React.FC<LayoutProps> = ({ ns, dispatch, children }) => {
  ...
  const createIngress = (payload: IngressRequest) => dispatch({ type: 'ingress/create', payload });
  const editIngressRules = (payload: ModifyIngressRulesType) => dispatch({ type: 'ingress/modify', payload });
  useEffect(() => { !!ns && getIngresss() }, [ns]);
  return (
    <Context.Provider value={{ namespace: ns, createIngress, getSecrets, getServices, editIngressRules }}>
      {children}
    </Context.Provider>
  )
}
