...
const NamespaceLayout: React.FC<Props> = ({
  loading,
  error,
  ns,
  nslist,
  ...
  const getNSs = () =>
    ...
    addNS = (payload: string) =>
      dispatch({type: 'namespace/create', payload}).then((error: any) => {
        !error && goto!(payload);
      });
  useEffect(() => {
    getNSs();
      ...
      {error ? (
        ...
        >
          <Empty description={error}>
            <Button loading={loading} type="primary" onClick={getNSs}>
              重试
            </Button>
          </Empty>
        </div>
...
const Namespace = ({loading, ns, nslist, goto, dispatch}: Props) => {
  ...
  const addNS = (payload: string) =>
    dispatch({type: 'namespace/create', payload}).then((error: any) => {
      !error && goto!(payload);
    });
  const deleteNS = (payload: string) =>
    dispatch({type: 'namespace/delete', payload});
  if (!!nslist[0] && nslist.every(v => v.metadata.name !== ns))
