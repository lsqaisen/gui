  ...
  return (
    <>
      ...
      <PageHeader
        ...
        tags={[<Tag>{`${({ public: '公网', internal: '内网' } as any)[ingress.network_type] || '未知'}`}</Tag>]}
        extra={[
          <Button key="edit" style={{ marginBottom: 16 }} type="primary" onClick={() => setVisible(true)}>编辑</Button>,
            ...
            <Tabs.TabPane key="1" tab="规则列表">
              <section style={{ padding: '16px 0' }}>
                <Table columns={[{
                  ...
                  dataIndex: 'port',
                  render: text => <a>{text}</a>,
                }]} dataSource={ingress.rules.map(v => ({ key: JSON.stringify(v), ...v }))} />
              </section>
            </Tabs.TabPane>
      ...
      >
        ...
        <Edit data={ingress} visible={visible} onClose={() => setVisible(false)} submit={editIngressRules} />
      </PageHeader>
    </>
  )
}
