...
const NameInput = forwardRef(({ value, onChange, ...props }: InputProps, ref: any) => {
  ...
  return (
    <>
      <Input ref={ref} className="nameinput" value={_value.join('-')} {...props} onChange={(e) => onChange!(`lb-${e.target.value}` as any)} prefix={<span>lb-</span>} />
      <style>{`
...
@(Form.create() as any)
class AddIngressForm extends PureComponent<FormComponentProps & IngressFromProps, any> {
  ...
  render() {
    ...
    const { name, namespace, network_type = "public", listen_http = true, listen_https = false, secret, rules } = (value || {} as IngressRequest)
    return (
      <Form>
        <FormItem
          {...formItemLayout}
        ...
        <FormItem
          ...
          {getFieldDecorator('listen_https', {
            ...
            <Checkbox onChange={(e) => {
              if (e.target.checked) {
                setFields({ secret: { value: undefined, errors: [Error("HTTPS启用时必须选择证书")] } })
