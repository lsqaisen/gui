...
class ContentInput extends React.PureComponent<any, any> {
		...
		if (files.size > 0 && files.size <= 1024 * 1024) {
			...
			reader.onload = (e: any) => {
				Modal.confirm({
					...
					okText: '追加',
					cancelText: '覆盖',
					onOk: () => setFieldsValue({ data: `${value ? `${value}\n` : ''}${e.target.result}` }),
	...
	render() {
		...
		return (
			<>
				<RadioGroup key="type" value={contentType} onChange={(e) => this.setState({ contentType: e.target.value })}>
					<Radio value="yaml">YAML</Radio>
					<Radio value="data">可视化</Radio>
				</RadioGroup>
				<div key="loadfile">
					<label style={{ lineHeight: '32px' }}>
						<input id="yaml" type="file" style={{ display: 'none' }} onChange={this.readFiles} />
						<a>加载本地文件</a><span>(不超过1M)</span>
					</label>
				</div>

				{contentType === 'yaml' ? <YamlInput {...this.props} /> : <Labels {...this.props} />}
			</>
		)
	}
}
