...
class VolumeInput extends React.PureComponent<VolumeInputProps, StateProps> {
  ...
  render() {
    ...
    switch (type) {
      case 'host':
        if (JSON.stringify(getFieldValue('host_path') || {}) !== JSON.stringify(host_path || {})) {
          setFieldsValue({ host_path })
        }
        break;
      case 'conf':
        if (JSON.stringify(getFieldValue('config_map') || {}) !== JSON.stringify(config_map || {})) {
          setFieldsValue({ config_map })
        }
        break;
      case 'secret':
        if (JSON.stringify(getFieldValue('secret') || {}) !== JSON.stringify(secret || {})) {
    ...
    return (
      <Row gutter={8}>
        <Col span={6}>
          <FormItem>
            <Select value={type} onChange={(type: any) => this.setState({ type })}>
        ...
        <Col span={12}>
          ...
          </FormItem> : type === "conf" ? <FormItem>
            {getFieldDecorator('config_map', {
              initialValue: config_map,
              rules: [{
                validator: (rule, value: Volume['config_map'], callback) => {
                  if (!value || !value.name) {
