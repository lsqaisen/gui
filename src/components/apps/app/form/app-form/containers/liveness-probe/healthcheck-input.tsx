...
class LivenessProbeInput extends PureComponent<LivenessProbeInputProps, any> {
  ...
  constructor(props: LivenessProbeInputProps) {
    super(props);
    this.state = {
      protocol: props.value!.exec ? "CMD" : props.value!.tcpSocket ? "TCP" : "HTTP"
    }
  }
  componentDidMount() {
  ...
  render() {
    ...
    const { tcpSocket, httpGet, exec, initialDelaySeconds, periodSeconds, timeoutSeconds, successThreshold, failureThreshold } = value!;
    ...
    return (
      <Card>
        <FormItem
          {...formItemLayout}
          label="检测协议">
          <Radio.Group value={protocol} onChange={(e) => {
        ...
        <FormItem
          ...
          {getFieldDecorator('initialDelaySeconds', {
            ...
            <InputNumber style={{ width: 180 }} placeholder="请输入初始化延时时间" />
          )} 秒
          </FormItem>
        <FormItem
          {...formItemLayout}
