...
class ReadOnly extends React.PureComponent<SelectProps, any> {
  render() {
    ...
    return (
      <Select {...props} defaultValue={value ? 1 : 0} onChange={(value: any, options: any) => onChange(!!value as any, options)}>
...
class VolumeMountInput extends React.PureComponent<VolumeMountInputProps, any> {
  ...
  render() {
    ...
    if (getFieldError('mount_path') && !!mount_path && others.every(v => v.mount_path !== mount_path)) {
        ...
        <Col span={7}>
          <FormItem>
            {getFieldDecorator('name', {
              initialValue: name,
              rules: [
                { required: true, message: '必须选择数据卷' },
              ],
            })(
              <Select placeholder="选择挂载卷">
                {volumes.filter(v => v.name).map(v => (<Select.Option key={v.name}>{v.name}</Select.Option>))}
              </Select>
            )}
          </FormItem>
        </Col>
              ...
              rules: [
                { required: true, message: '必须填写' }, {
                  validator: (_, value, callback) => {
                    if (others.some(({ mount_path }) => !!value && mount_path === value)) {
                      callback('存在相同的挂载路径')
                    }
                    callback()
                  }
