...
export default class extends React.PureComponent<ImageInputProps, any> {
  ...
  render() {
    ...
    return (
      <Context.Consumer>
        {({ getImages, getTags }: ContextProps) => <Row gutter={8}>
          <Col style={{ width: 'calc(100% - 42px)', float: "left" }}>
            {type === 'select' ? <Row gutter={8}>
              <Col span={16}>
                <FormItem extra={`搜索针对于当前下拉显示的数据，默认显示10条数据，下拉滚动可加载更多数据`}>
                  {getFieldDecorator('image_name', {
                    ...
                    <SearchSelect
                      ...
                      asyncSearch={async (page, callback) => {
                        const { items, total }: any = await getImages!({ domain: 'on', page, size: 10 });
                        callback({
                          total,
                          results: items.map((image: any) => ({
                            key: image.name,
                            label: <Tooltip title={image.name}>{image.name.split('\/').pop()}</Tooltip>,
                          }))
                        })
                      }}
                    />
              ...
              <Col span={8}>
                  ...
                  {getFieldDecorator('image_tag', {
                    ...
                    <SearchSelect
                      ...
                      asyncSearch={async (_, callback) => {
                        const response: any[] = await getTags!((image || '').split('\/').slice(1).join('/'));
                        callback({
                ...
                <FormItem
                  extra="请输入完整镜像地址，支持输入外部仓库中的镜像，例如：example.com/dev/app1:v2"
                >
