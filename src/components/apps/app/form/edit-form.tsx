...
class EditForm extends React.PureComponent<EditFormProps, any> {
  ...
  render() {
    ...
    const { name, namespace, type = "Deployment", replicas, labels, volumes, containers = [{}], service } = (data || {} as App)
    ...
    return (
      <Form>
        ...
        <FormItem
          labelCol={{ span: 24 }}
          wrapperCol={{ span: 24 }}
          label={<Typography.Text strong style={{ fontSize: '16px' }}>实例内容器</Typography.Text>}
          validateStatus=""
          help="">
          {getFieldDecorator('containers', {
            ...
            <Containers labels={['name', 'image', 'env']} volumes={getFieldValue('volumes') || []} />
          )}
        </FormItem>
      </Form>
    )
