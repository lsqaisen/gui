      ...
      return (
        <Form.Item>
          {getFieldDecorator('replicas', {
            initialValue: value,
            rules: [
              { required: true, message: '实例数必须填写' },
            ],
          })(
            <InputNumber style={{ width: '100%' }} placeholder="请输入实例数" min={1} />
          )}
        </Form.Item>
      )
    }
...
export default ({ btn, data, onOk, callback = () => null, ...props }: DeleteProps) => {
  let form: WrappedFormUtils;
  function onClick() {
    Modal.confirm({
      title: `修改服务${data.name}实例数`,
      content: (
        <Replicas value={data.replicas} ref={(ref: any) => ref && (form = ref)} />
      ),
      okText: '确认',
      cancelText: '取消',
  ...
  return !btn ? <Button onClick={onClick} >删除</Button> : cloneElement(btn as any, { ...props, onClick })
}