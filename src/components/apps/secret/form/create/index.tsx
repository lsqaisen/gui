...
const Labels = Inputs.Wapper({
  load: (value) => Object.entries(value || {}).map(([key, value]: any) => ({ key, value })),
  dump: (value) => {
...
class CreateForm extends React.PureComponent<CreateFormProps, any> {
  ...
  render() {
    ...
    const { name, namespace, type = "Opaque", stringData } = (value || {} as SecretRequest)
    return (
      <Form>
        <FormItem
          labelCol={labelCol}
        ...
        <FormInput
          ...
          label="内容"
        >
          {getFieldValue('type') === 'Opaque' ? getFieldDecorator('stringData', {
