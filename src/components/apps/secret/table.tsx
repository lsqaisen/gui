    ...
    <Fragment>
      <Descriptions title={<Typography.Text style={{ fontWeight: 'bold', fontSize: '16px' }} copyable={{ text }}>配置项</Typography.Text>}>
        {Object.entries(data || {}).map(([key, value]: any) => (<Descriptions.Item span={3} key={key} label={key}>
          <Typography.Paragraph style={{ wordBreak: 'break-all' }} ellipsis={{ rows: 3 }} copyable={{ text: `${key}:${value}` }}>{value}</Typography.Paragraph>
        </Descriptions.Item>))}
      </Descriptions>
      <Divider />
...
class SecretTable extends PureComponent<SecretTableProps, any> {
  columns: ColumnProps<ISecret>[] = [
    ...
    },
    {
      ...
      render: (t, r) => <Time format="YYYY-MM-DD  HH:mm" value={new Date(r.metadata.creationTimestamp)} />,
    },
  ];
  state = {
    visible: false,
