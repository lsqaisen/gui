    ...
    <Basic
      ...
      tooltipContent={({payload}) => {
        ...
        return (
          ...
          >
            <h3>
              {`${(Array(2).join(0) + Number(time.getMonth() + 1)).slice(
                -2
              )}/${(Array(2).join(0) + Number(time.getDate())).slice(-2)}  ${(
                Array(2).join(0) + Number(time.getHours())
              ).slice(-2)}:${(
                Array(2).join(0) + Number(time.getMinutes())
              ).slice(-2)}`}
            </h3>
