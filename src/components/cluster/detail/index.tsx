    ...
    return (
      <Fragment>
        ...
        <PageHeader
          ...
          subTitle={(node!.addresses.find((v: any) => v.type === "InternalIP") || { address: '' }).address}
          tags={[<Tag
            key="status"
          ...
          extra={[
            <Button key="3" onClick={getDode}>刷新</Button>
          ]}
          footer={
            <Tabs style={{ padding: '0 16px' }} defaultActiveKey="1">
              ...
              <TabPane tab="其他信息" key="other" >
                <Typography style={{ padding: '0 8px' }}>
                  ...
                  <Paragraph>
                    {Object.entries(node!.metadata.labels || {}).map(([key, value]: any) => <Description key={key} term={key} span={24}><Paragraph copyable={{ text: `${key}:${value}` }}>{value}</Paragraph></Description>)}
                  </Paragraph>
                  <Title level={2}>NodeOther</Title>
                  <Paragraph>
                    {Object.entries(node!.metadata.annotations || {}).map(([key, value]: any) => <Description key={key} term={key} span={24}>{value}</Description>)}
                  </Paragraph>
                </Typography>
              </TabPane>
            </Tabs>
            ...
            <Col className={styles.content} xs={24} lg={12}>
              <Row gutter={24}>
                ...
                <Description term="内存" xs={24} md={12}>
                  <Statistic
                    suffix={` / ${((node!.capacity.memory || 0) / (1024 * 1024)).toFixed(1)}GB`}
                    value={((node!.allocatable.memory || 0) / (1024 * 1024)).toFixed(1)}
                  />
                </Description>
