  ...
  columns: ColumnProps<INode>[] = [
    {
      ...
      render: (_, r) => {
        let text = `${r.metadata.name}<${(r.addresses.find(v => v.type === "InternalIP") || { address: '--' }).address}>`;
        return (
      ...
      render: (_, r) => (
        <Chart
          color="#5380ea"
          extra={`${r.capacity.cpu ? `${r.capacity.cpu / 1000}核` : '--'}/${r.allocated.cpu / 1000}核`}
      ...
      render: (_, r) => (
        <Chart
          color="#3fc3cf"
          extra={`${r.capacity.memory ? `${window.Number(r.capacity.memory).flowCeil(2)}` : '--'}/${window.Number(r.allocated.memory).flowCeil(2)}`}
          percent={r.capacity.memory / 1024 / 1024 / 1024}
          symbol="Gi"
          data={[{
            title: 'mem',
            actual: Number(Number(((r.usages || {}).memory || 0) / 1024 / 1024 / 1024).toFixed(2)),
            request: Number(Number(r.allocated.memory / 1024 / 1024 / 1024).toFixed(2)),
          }]}
        />
      )
    }, {
