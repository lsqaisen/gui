...
import { Icon, Drawer, Button, Tooltip } from 'antd';
...
const Install = ({ onFullScreen, ...props }: InstallProps) => {
  ...
  useEffect(() => {
    visible && onFullScreen(fullscreen, (term as Terminal))
  }, [fullscreen])
  return (
    <>
      <Button style={{ cursor: 'pointer', padding: 0 }} type="link" onClick={() => setVisible(true)}> <Icon style={{ margin: 0 }} type="sync" spin />正在安装</Button>
      <Drawer
        {...props}
      ...
      >
        <Loading loading={!show}>
          <Terminal
            ref={(ref) => !!ref && setTerm(ref)}
            splitWrite={true}
            bidirectional={false}
            buffered={true}
            beforeSendData={["hello word"]}
            url={`${ishttps ? 'wss' : 'ws'}://${window.location.host.includes("localhost") ? `localhost:${ishttps ? '6600' : '6601'}` : window.location.host}/api/cluster/logs/install`}
          />
        </Loading>
      </Drawer>
    </>
