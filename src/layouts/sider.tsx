...
export default ({ onEnd }: SiderProps) => (
  <Context.Consumer>
    {({ profile = {} as IUser, pathname, menus, logout = () => null, modifyPassword = () => null }) => (
      <div style={{ height: '100%', width: "100%" }}>
        <section style={{ height: 64, padding: 8 }}>
          <Logo
            iconSrc={`/dist/oem${process.env.NODE_ENV === "development" ? process.env.OEM_NAME : ''}/icon.png`}
            logoSrc={`/dist/oem${process.env.NODE_ENV === "development" ? process.env.OEM_NAME : ''}/logo.png`}
          />
        </section>
        ...
        <div style={{ lineHeight: '32px', textAlign: 'center', borderTop: '1px solid #f8f8f8' }}> build <Time format="YYYY-MM-DD  HH:mm" value={new Date(Number(process.env.VERSION))} /></div>
      </div>
    )}
  </Context.Consumer>
)